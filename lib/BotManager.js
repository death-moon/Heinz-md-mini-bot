...
class BotManager extends EventEmitter {
  ...
  async initialize() {
    try {
      ...
      this.logger.info("*✅ ʙᴏᴛᴍᴀɴᴀɢᴇʀ ɪɴɪᴛɪᴀʟɪsᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s*");
      return { success: true };
    } catch (error) {
      this.logger.error(
        "*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟ'ɪɴɪᴛɪᴀʟɪsᴀᴛɪᴏɴ ᴅᴜ ʙᴏᴛᴍᴀɴᴀɢᴇʀ:*",
        error,
      );
      throw error;
    }
  ...
  async createBot(number, userSettings = {}) {
    ...
    if (this.bots.has(sanitizedNumber)) {
      return {
        success: false,
        message: "*ᴜɴ ʙᴏᴛ ᴇsᴛ ᴅᴇ́ᴊᴀ̀ ᴀᴄᴛɪғ ᴘᴏᴜʀ ᴄᴇ ɴᴜᴍᴇ́ʀᴏ*",
      };
    }
    ...
    try {
      ...
      this.logger.info(`*🤖 ʙᴏᴛ ᴄʀᴇ́ᴇ́ ᴘᴏᴜʀ ${sanitizedNumber}*`);
      return { success: true, number: sanitizedNumber };
    } catch (error) {
      this.logger.error(
        `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟᴀ ᴄʀᴇ́ᴀᴛɪᴏɴ ᴅᴜ ʙᴏᴛ ᴘᴏᴜʀ ${sanitizedNumber}:*`,
        error,
      );
      return {
        success: false,
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        ...
        if (attempts < config.MAX_RECONNECT_ATTEMPTS) {
          ...
          this.logger.info(
            `*🔄 ᴛᴇɴᴛᴀᴛɪᴠᴇ ᴅᴇ ʀᴇᴄᴏɴɴᴇxɪᴏɴ ${attempts + 1}/${config.MAX_RECONNECT_ATTEMPTS} ᴘᴏᴜʀ ${number} ᴅᴀɴs ${delay}ᴍs*`,
          );
          ...
          this.logger.error(
            `*❌ Échec de ʀᴇᴄᴏɴɴᴇxɪᴏɴ ᴀᴘʀᴇ̀s ${config.MAX_RECONNECT_ATTEMPTS} ᴛᴇɴᴛᴀᴛɪᴠᴇs ᴘᴏᴜʀ ${number}*`,
          );
          await this.stopBot(number);
        }
      } else {
        this.logger.info(`*🔌 ᴅᴇ́ᴄᴏɴɴᴇxɪᴏɴ ᴅᴇ́ғɪɴɪᴛɪᴠᴇ ᴘᴏᴜʀ ${number}*`);
        await this.stopBot(number);
      }
      ...
      this.logger.info(`*✅ ʙᴏᴛ ᴄᴏɴɴᴇᴄᴛᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s: ${botInfo?.id || number}*`);
      ...
      if (bot.settings.AUTO_JOIN_GROUP && bot.settings.SUPPORT_GROUP) {
        try {
          ...
          this.logger.info(`*📱 ᴀᴜᴛᴏ-ᴊᴏɪɴ ɢʀᴏᴜᴘᴇ sᴜᴘᴘᴏʀᴛ ʀᴇ́ᴜssɪ ᴘᴏᴜʀ ${number}*`);
        } catch (error) {
          this.logger.warn(
            `*⚠️ ᴀᴜᴛᴏ-ᴊᴏɪɴ ɢʀᴏᴜᴘᴇ sᴜᴘᴘᴏʀᴛ ᴇ́ᴄʜᴏᴜᴇ́ ᴘᴏᴜʀ ${number}: ${error.message}*`,
          );
        }
      }
  ...
  async handleMessages(number, { messages, type }) {
    ...
    for (const msg of messages) {
      try {
        ...
        this.logger.error(
          `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴜ ᴛʀᴀɪᴛᴇᴍᴇɴᴛ ᴅᴜ ᴍᴇssᴀɢᴇ ᴘᴏᴜʀ ${number}:*`,
          error,
        );
      }
    }
    ...
    for (const call of calls) {
      ...
      if (bot.settings.AUTO_REJECT_CALLS) {
        ...
        await bot.socket.sendMessage(call.from, {
          text: "*🚫 ʟᴇs ᴀᴘᴘᴇʟs sᴏɴᴛ ᴀᴜᴛᴏᴍᴀᴛɪǫᴜᴇᴍᴇɴᴛ ʀᴇᴊᴇᴛᴇ́s. ᴠᴇᴜɪʟʟᴇᴢ ᴇɴᴠᴏʏᴇʀ ᴜɴ ᴍᴇssᴀɢᴇ ᴛᴇxᴛᴇ.*",
        });
      }
  ...
  startPresenceUpdates(socket, number, settings) {
    const interval = setInterval(async () => {
      try {
        ...
        this.logger.error(
          `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟᴀ ᴍɪsᴇ ᴀ̀ ᴊᴏᴜʀ ᴅᴇ ᴘʀᴇ́sᴇɴᴄᴇ ᴘᴏᴜʀ ${number}:*`,
          error,
        );
      }
  ...
  async sendStartupMessage(socket, ownerNumber) {
    try {
      const message =
        `🚀 *ʜᴇɪɴᴢ-ᴍᴅ ʙᴏᴛ ᴠ${config.BOT_VERSION}*\n\n` +
        `*✅ ʙᴏᴛ ᴅᴇ́ᴍᴀʀʀᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s!*\n` +
        `*📅 ᴅᴀᴛᴇ: ${moment().tz("Africa/Douala").format("DD/MM/YYYY HH:mm:ss")}*\n\n` +
        `*💡 ᴛᴀᴘᴇᴢ *.menu* ᴘᴏᴜʀ ᴠᴏɪʀ ʟᴇs ᴄᴏᴍᴍᴀɴᴅᴇs ᴅɪsᴘᴏɴɪʙʟᴇs*`;
      ...
      this.logger.error(
        "*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟ'ᴇɴᴠᴏɪ ᴅᴜ ᴍᴇssᴀɢᴇ ᴅᴇ ᴅᴇ́ᴍᴀʀʀᴀɢᴇ:*",
        error,
      );
    }
  }
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        const timeout = setTimeout(() => {
          reject(new Error("*ᴛɪᴍᴇᴏᴜᴛ: ᴀᴜᴄᴜɴ ᴄᴏᴅᴇ ᴅᴇ ᴘᴀɪʀɪɴɢ ɢᴇ́ɴᴇ́ʀᴇ́*"));
  ...
  async stopBot(number) {
    ...
    if (!bot) {
      return {
        success: false,
        message: "*ᴀᴜᴄᴜɴ ʙᴏᴛ ᴀᴄᴛɪғ ᴘᴏᴜʀ ᴄᴇ ɴᴜᴍᴇ́ʀᴏ*",
      };
    }
    ...
    try {
      ...
      this.logger.info(`*🛑 ʙᴏᴛ ᴀʀʀᴇ̂ᴛᴇ́ ᴘᴏᴜʀ ${sanitizedNumber}*`);
      this.emit("bot-stopped", { number: sanitizedNumber });

      return {
        success: true,
        message: "*ʙᴏᴛ ᴀʀʀᴇ̂ᴛᴇ́ ᴀᴠᴇᴄ sᴜᴄᴄᴇ̀s*",
      };
    } catch (error) {
      this.logger.error(
        `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟ'ᴀʀʀᴇ̂ᴛ ᴅᴜ ʙᴏᴛ ᴘᴏᴜʀ ${sanitizedNumber}:*`,
        error,
      );
      return {
        success: false,
  ...
  async stopAll() {
    ...
    this.logger.info("*🛑 ᴛᴏᴜs ʟᴇs ʙᴏᴛs ᴏɴᴛ ᴇ́ᴛᴇ́ ᴀʀʀᴇ̂ᴛᴇ́s*");
  }
  ...
  async backupAllSessions() {
    ...
    for (const [number, bot] of this.bots) {
      try {
        ...
        this.logger.info(`*💾 sᴇssɪᴏɴ sᴀᴜᴠᴇɢᴀʀᴅᴇ́ᴇ ᴘᴏᴜʀ ${number}*`);
      } catch (error) {
        this.logger.error(
          `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟᴀ sᴀᴜᴠᴇɢᴀʀᴅᴇ ᴅᴇ ʟᴀ sᴇssɪᴏɴ ᴘᴏᴜʀ ${number}:*`,
          error,
        );
      }
    }
  ...
  async restoreAllSessions() {
    ...
    if (!(await fs.pathExists(sessionsPath))) {
      return { success: false, message: "*ᴀᴜᴄᴜɴᴇ sᴇssɪᴏɴ ᴛʀᴏᴜᴠᴇ́ᴇ*" };
    }
    ...
    for (const session of sessions) {
      ...
      if (stats.isDirectory()) {
        try {
          ...
          this.logger.info(`*📱 sᴇssɪᴏɴ ʀᴇsᴛᴀᴜʀᴇ́ᴇ ᴘᴏᴜʀ ${session}*`);
        } catch (error) {
          this.logger.error(
            `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟᴀ ʀᴇsᴛᴀᴜʀᴀᴛɪᴏɴ ᴅᴇ ʟᴀ sᴇssɪᴏɴ ᴘᴏᴜʀ ${session}:*`,
            error,
          );
        }
      }
  ...
  async loadExistingSessions() {
    ...
    const sessions = await fs.readdir(sessionsPath);
    this.logger.info(`*📂 ${sessions.length} sᴇssɪᴏɴ(s) ᴛʀᴏᴜᴠᴇ́ᴇ(s)*`);
  }
      ...
      for (const [number, bot] of this.bots) {
        if (bot.status === "connected" && bot.socket) {
          try {
            ...
            this.logger.error(
              `*ᴇʀʀᴇᴜʀ ʟᴏʀs ᴅᴇ ʟᴀ ᴠᴇ́ʀɪғɪᴄᴀᴛɪᴏɴ ᴅᴜ sᴛᴀᴛᴜᴛ ᴘᴏᴜʀ ${number}:*`,
              error,
            );
          }
        }
  ...
  setupPairingManagerEvents() {
    this.pairingManager.on('pairing-success', async ({ number, info, sessionData }) => {
      this.logger.info(`*🎉 ᴊᴜᴍᴇʟᴀɢᴇ ʀᴇ́ᴜssɪ ᴘᴏᴜʀ ${number}*`);
    ...
    this.pairingManager.on('pairing-failed', ({ number, reason }) => {
      this.logger.error(`*❌ ᴇ́ᴄʜᴇᴄ ᴊᴜᴍᴇʟᴀɢᴇ ᴘᴏᴜʀ ${number}: ${reason}*`);
      this.emit('pairing-failed', { number, reason });
    });

    this.pairingManager.on('pairing-timeout', ({ number }) => {
      this.logger.warn(`*⏰ ᴛɪᴍᴇᴏᴜᴛ ᴊᴜᴍᴇʟᴀɢᴇ ᴘᴏᴜʀ ${number}*`);
